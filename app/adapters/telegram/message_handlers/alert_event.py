...
class AlertManager:
    ...
    def trigger_alert(self, event: TelegramMessageAdapter) -> None:
        """
        触发告警

        逻辑：
        1. 为每个user_name创建延迟任务（30秒后播放音效）
        ...
        if user_name in self._alert_tasks:
            alert_task = self._alert_tasks[user_name]
            ...
            if not alert_task.task.done():
                ...
                # 创建新的延迟任务
                new_task = asyncio.create_task(self._delayed_alert(user_name))
                alert_task.task = new_task
                logger.info(f"{user_name}: 更新告警延迟，30秒后播放音效")
            else:
                # 任务已完成，检查是否需要重新告警
                if alert_task.alert_state:
                    # 告警状态为true但任务已完成，说明音效已播放
                    alert_task.alert_state = False
                    logger.info(f"{user_name}: 音效已播放，告警状态置为false")
                else:
                    # 告警状态为false，未清理的任务，跳过不更新
                    logger.info(f"{user_name}: 任务已完成但未清理，跳过触发")
        else:
            # 新user_name，创建告警任务
            self._create_new_alert_task(user_name)
            logger.info(f"{user_name}: 创建告警任务，30秒后播放音效")

    def _create_new_alert_task(self, user_name: int) -> None:
        """创建新的告警任务"""
        task = asyncio.create_task(self._delayed_alert(user_name))
        ...
        self._alert_tasks[user_name] = alert_task

    async def _delayed_alert(self, user_name: int) -> None:
        """延迟告警执行"""
        await asyncio.sleep(self.delay_seconds)

        # 检查任务是否仍然存在且告警状态为true
        if user_name in self._alert_tasks and self._alert_tasks[user_name].alert_state:
            # 播放音效
            AUDIO_HANDLER.play_audio(volume=0.5, loop=3)

            # 播放完音效后，告警状态置为false
            self._alert_tasks[user_name].alert_state = False
            logger.info(f"{user_name}: 音效播放完成，告警状态置为false")

    def get_alert_state(self, user_name: int) -> Optional[bool]:
        """获取指定user_name的告警状态"""
        if user_name in self._alert_tasks:
            return self._alert_tasks[user_name].alert_state
        return None
    ...
    def cleanup_tasks(self) -> None:
        ...
        for user_name, alert_task in self._alert_tasks.items():
            ...
            if (
                ...
                if not alert_task.task.done():
                    alert_task.task.cancel()
                expired_chats.append(user_name)
                logger.info(f"{user_name}: 任务过期，已清理")

        # 清理过期任务
        for user_name in expired_chats:
            del self._alert_tasks[user_name]
...
if __name__ == "__main__":
    ...
    class MockMessage:
        def __init__(self, user_name):
            ...
            # self.user_name = f"User{user_name}"
    ...
    async def run_test():
        ...
        logger.info("1. 首次触发告警 - 123")
        alert_manager.trigger_alert(MockMessage(user_name=123))
        ...
        logger.info("\n5. 测试其他user_name")
        alert_manager.trigger_alert(MockMessage(user_name=456))
        alert_manager.trigger_alert(MockMessage(user_name=789))
